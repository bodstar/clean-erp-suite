...
export function SDScopeSelector() {
  const { hasPermission, teams } = useAuth();
  const { scopeMode, targetTeamId, setScopeMode, setTargetTeamId } = useSDScope();
  const isMobile = useIsMobile();

  const canViewAll = hasPermission("sd.hq.global_view");
  const canActOnTeam = hasPermission("sd.hq.act_any_team");
